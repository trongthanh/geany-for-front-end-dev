/**
 * Windows Script Host runner for JSHint
 * Edited by Thanh Tran <trongthanh AT gmail DOT com>
 * Release date: 2012-06-25
 *
 * Adapted for Geany 1.22+ and JSHint after 2011-04-16
 * Use this Error RegExp in Geany: (.+):\ line\ ([0-9]+)
 */
	...
	function processScript(script) {
		...
		for (var formatter in formatters) {
			if (data[formatter]) {
                ...
                data[formatter].scriptName = currentScriptName;
	...
	var formatters = {
		errors: function(errors, lines) {
			...
			for (var i = 0; i < errors.length; i++) {
				...
				lines.push(errors.scriptName + ": line " + error.line + " col " + error.character + ": " + error.reason);
		...
		implieds: function(implieds, lines) {
			...
			for (var name in globals) {
                ...
                var itemLines = globals[name][0];
                for (i = 0; i < itemLines.length; i++) {
                    lines.push(implieds.scriptName + ": line " + itemLines[i] + ": " + name);
                }
			}
		},
		...
		unused: function(unused, lines) {
			...
			for (i = 0; i < unused.length; i++) {
				...
				names[func].push({name: item.name, line: item.line});
			...
			for (func in names) {
                var itemLines = names[func];
                for (i = 0; i < itemLines.length; i++) {
                    lines.push(unused.scriptName + ": line " + itemLines[i].line + ": " + func + ": " + itemLines[i].name);
                }
			}
		}
	};
